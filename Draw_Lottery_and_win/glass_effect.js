...
nav1.addEventListener('click',function(){
    if(user.className=="lgn_after"){
         user.className="user_portion";
    }
    else{
        user.className="lgn_after";
...
function validate() {
    ...
    if(document.myform.user_mail.value==""){
        alert("Please Provide Your Mail Or Phone Number");
        document.myform.eml_num.focus();
        return falsel;
    }
    var email_id=document.myform.user_mail.value;
...
eimg_bt.addEventListener('click',function(){
     if(eimg_bt.innerHTML=="Edit Image" && edit_image.className=="edit_before"){
    ...
    else if(edit_image.className=="edit_img"){
        edit_image.className="edit_before";
    }
    else {
...
//////
const img_element=[document.getElementById('l7'),
                document.getElementById('l8'),
                document.getElementById('l5')];

const bt_element=[document.getElementById('buy-silver'),
                document.getElementById('buy-gold'),
                document.getElementById('buy-diamond')]
var transfer_img="";
for(let i=0;i<img_element.length;i++){
    bt_element[i].addEventListener('click',function(){
        transfer_img=img_element[i].getAttribute('src');
        sessionStorage.setItem("myImage",transfer_img);
        window.location.href="draw_lottey.html";
    })
}
